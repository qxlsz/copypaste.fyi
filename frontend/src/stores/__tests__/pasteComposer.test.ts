...
describe("usePasteComposerStore", () => {
  ...
  it("computes effective retention from preset selection", () => {
    usePasteComposerStore.setState({
      selectedRetention: 60,
      customRetention: "",
    });
  ...
  it("records toast metadata", () => {
    usePasteComposerStore
      .getState()
      .recordToast({ title: "Created", tone: "success" });
    expect(usePasteComposerStore.getState().lastToast).toEqual({
      title: "Created",
      tone: "success",
    });
  });

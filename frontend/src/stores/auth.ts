...
// Remove credentials written by older releases. Current sessions live only in
// this module's Zustand store and therefore require a fresh login after reload.
try {
...
export const useAuth = create<AuthState>()((set, get) => ({
    ...
    try {
      ...
      switch (format) {
        ...
        case "pem": {
          ...
          if (!pemMatch) {
            throw new Error(
              "Invalid PEM format - expected Ed25519 private key",
            );
          }
      ...
      if (privkeyBytes.length !== 32) {
        throw new Error(
          `Invalid key length: ${privkeyBytes.length} bytes (expected 32 bytes for Ed25519)`,
        );
      }
  ...
  logout: async () => {
    ...
    try {
      ...
      // The local session must still be destroyed if the backend cannot
      // confirm logout. The warning below makes that state explicit.
    } finally {
      // Session material is memory-only and is always cleared locally,
      // including when the backend is unavailable.
      set({ user: null, token: null });
    }

    if (token && !serverLogoutConfirmed) {
      toast.warning("Local session cleared", {
        description: "Server logout could not be confirmed.",
      });
    } else {

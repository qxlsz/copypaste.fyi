...
export const validateEncryptionKey = (key: string): string | null => {
  ...
  if (
    ...
    upper.includes("BEGIN RSA PUBLIC KEY") ||
    upper.includes("BEGIN RSA ENCRYPTED PRIVATE KEY")
  ) {
    return "RSA keys are not used here. Generate a 256-bit secret, or pick ML-KEM hybrid.";
  }
  return null;
};
...
export const detectStegoImageMime = (bytes: Uint8Array): SupportedStegoMime => {
  if (hasSignature(bytes, PNG_SIGNATURE)) return "image/png";
  if (hasSignature(bytes, BMP_SIGNATURE)) return "image/bmp";
  throw new StegoImageValidationError("Only genuine PNG or BMP carrier images are supported.");
};
...
export const readStegoImage = async (
  ...
  if (file.size > MAX_STEGO_IMAGE_BYTES) {
    throw new StegoImageValidationError("Carrier images must be 1 MiB or smaller.");
  }

...
export const AreaGroupChart = ({ data, formatLabel, height = 240 }: AreaGroupChartProps) => {
  const computed = useMemo<ChartComputed | null>(() => {
    if (data.length === 0) {
      return null
    }

    const parsed = data.map((item) => ({ date: new Date(item.date), value: item.value }))
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Pastes created over time" className="w-full">
      <defs>
      ...
      <path d={pathData} fill={`url(#${gradientId})`} stroke="#6366f1" strokeWidth={2} />

      {xTicks.map(({ tick, x }) => (
        <g key={`x-${tick.toISOString()}`} transform={`translate(${x}, ${height - margin.bottom})`}>
          <line x1={0} x2={0} y1={0} y2={6} stroke="#334155" />
          <text dy={16} textAnchor="middle" className="fill-gray-400 text-[10px]">
      ...
      {yTicks.map(({ tick, y }) => (
        <g key={`y-${tick}`} transform={`translate(${margin.left}, ${y})`}>
          <line x1={0} x2={width - margin.left - margin.right} y1={0} y2={0} stroke="#1e293b" />
          <text x={-8} dy={4} textAnchor="end" className="fill-gray-400 text-[10px]">
            {tick}
          </text>
        </g>
      ))}
